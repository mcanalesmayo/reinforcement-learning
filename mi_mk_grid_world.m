function [T, T2, T1] = mi_mk_grid_world(nrows, ncols, psucc_act, obstacle, terminal, absorb, wrap_around, noop)
...
% Se obtiene a partir de T2 y T1.

% T2{a}(s,s')=r means s->s' with prob r when we apply action 'a'.
% This is computed by assigning prob mass p to the cell 'a' (N north for example) of s,
...
% T1{a}(i,j)=k means s->k when we apply action 'a', where s = encode_xy(i,j)
...
% dir = [N E S W; E N W S; S E N W; W N E S];
% p = psucc_act;
% q = (1-p)/3;
% prob = [p q q q; p q q q; p q q q; p q q q];
